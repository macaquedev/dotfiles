...
ProgressBar {
    ...
    // Optional warning overshoot: fill beyond `warningThreshold` (in value units)
    // is drawn in `warningColor`. A negative threshold disables it entirely.
    property real warningThreshold: -1
    property color warningColor: Appearance?.m3colors?.m3error ?? "#E05A4F"
    ...
    contentItem: Item {
        ...
        Loader {
            ...
            sourceComponent: Item {
                ...
                Rectangle { // Full fill, drawn in warningColor (red) underneath
                ...
                Rectangle { // Normal fill on top, capped at the warning threshold so
                            // any overshoot beyond it shows the red underneath.
                    anchors.left: parent.left
                    property real thresholdPos: (root.warningThreshold >= 0 && root.to > root.from)
                        ? (root.warningThreshold - root.from) / (root.to - root.from)
                        : 1
                    width: contentItem.width * Math.min(root.visualPosition, thresholdPos)
                    height: contentItem.height
                    radius: Appearance.angelEverywhere ? Appearance.angel.roundingSmall : height / 2
                    color: root.highlightColor
                }
