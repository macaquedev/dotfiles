...
# 1. keyd — OPTIONAL system-wide key remapping. Done FIRST and behind an upfront
#    sudo check, so anyone who wants it is told immediately that it needs root
#    (and the sudo prompt happens before the long iNiR install, not midway).
...
if [[ "$install_keyd" -eq 1 ]]; then
  # keyd writes to /etc/keyd and enables a system service, so it needs root.
  # Verify sudo NOW and fail loudly if it isn't available.
  if ! sudo -v; then
    echo "!! keyd needs sudo to write /etc/keyd and enable the service, but sudo" >&2
    echo "   isn't available for you. Re-run as a sudo-capable user, or pass" >&2
    echo "   --no-keyd to skip it. Aborting." >&2
    exit 1
  fi
  ...
  # a previous .bak either), then write mine.
...
# 2. iNiR base (compositor deps, quickshell runtime, fonts, venv, the `inir` CLI)
...
# 3. Overlay this repo's config on top
...
# 4. Resolve the home placeholder to this user's real home
...
# 5. ydotool — the bar uses it to warp the cursor back after a workspace click
...
# 6. Enable user services + reload niri
